...
bot.on("text", async (ctx) => {
  ...
  if (pe) {
    ...
    if (pe.mode === "field") {
      ...
      await ctx.reply(`✅ Salvo com sucesso: ${fieldLabel(pe.field)} = ${String(value).trim()}`);
  ...
  try {
    ...
    if (media_group_id) {
      ...
      const t = setTimeout(async () => {
        const msgs = pendingMediaGroups.get(media_group_id) || [];
        pendingMediaGroups.delete(media_group_id);
        pendingMediaGroupTimers.delete(media_group_id);

        if (!msgs.length) return;

        // Processa o lote inteiro em UMA única revisão (evita 3 mensagens)
        try { await ctx.reply("🧠 Lendo lote..."); } catch {}

        const book_hint = ctx.session?.lastBook || "";
        const items = [];

        for (const msg of msgs) {
          try {
            const r = await processMessage(msg, book_hint);
        ...
        if (!items.length) {
          await ctx.reply("❌ Não consegui ler esse lote. Tente enviar fotos mais nítidas/mais perto.");
          return;
        }
...
const EDIT_FIELD_UI = {
  book:   { label: "Casa",      prompt: "Qual é o nome da casa de apostas? (ex: betano, bet365, lotu)" },
  stake:  { label: "Stake",     prompt: "Qual valor da sua stake? (ex: 50 ou 2% da banca)" },
  event:  { label: "Descrição", prompt: "Qual é a descrição/jogo da aposta? (ex: Grêmio x Palmeiras)" },
  market: { label: "Mercado",   prompt: "Qual mercado? (ex: Ambas marcam, Over 2.5, Handicap +0.5)" },
  odd:    { label: "Odds",      prompt: "Qual odd (decimal)? (ex: 1.85)" },
  sport:  { label: "Esporte",   prompt: "Qual esporte? (ex: Futebol, NBA, Tênis)" },
};
